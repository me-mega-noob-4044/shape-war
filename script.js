(function () {
    ...
    - Red Circle Built-In Arc Reactor: 35% -> 15%<br>
    - Import and Exporting! Back up your progress!<br>
    - Nucleon Damage: -20%<br>
    - Offers: Free rewards!<br>
    - New Settings Feature: import and exporting your progress, or restarting everything!<br><br>
    ...
    var shapeData = [{
        ...
        250% of damage taken will be redirected to the attacker.<br><br>
        ...
        Shape comes with built-in arc reactor, increasing its damage by 15%.<br><br>
    ...
    var weaponData = [{
        ...
        imageSource: "./images/atomizer.png",
        fireRate: 50,
        ammo: 1e300,
        reload: 0,
        range: 1200,
        ...
        imageSource: "./images/nucleon.png",
        fireRate: 75,
        ammo: 1e300,
        reload: 0,
        range: 1200,
    ...
    function updateMoneyDisplay(index, value) {
        ...
        Silver: ${abbreviateNumber(player.sliver)} | League: ${abbreviateNumber(player.league)}<br>
    ...
    function resize() {
        ...
        document.getElementById("PROAD").style.top = (window.innerHeight/2-200) + "px";
        document.getElementById("PROAD").style.left = (window.innerWidth/2-200) + "px";
    ...
    document.getElementById("offersToggles").onclick = function() {
        if(offers.length) {
            ...
            for(let i = 0; i < offers.length; i++) {
                ...
                <div style="position: absolute; top: 0px; left: ${400*i}px; width: 400px; height: 400px; background-color: ${i % 2 ? "rgb(255, 255, 255, 0.4)" : "rgb(0, 0, 0, 0.2)"}; background-size: 400px; 400px; background-image: url('${offers[i].type == "silver" ? agSource : offers[i].type == "gold" ? auSource : keysSource}');">
                <div style="position: absolute; top: 5px; left: 10px; font-size: 30px;">
            ...
            for(let i = 0; i < offers.length; i++) {
                document.getElementById(`watchAds${i}`).onclick = function() {
                    ...
                    document.getElementById("PROAD").currentTime = 0;
                    document.getElementById("PROAD").play();
                    setTimeout(() => {
                        document.getElementById("ADDS YAY").style.display = "none";
                        offers[i].ads++;
                        document.getElementById("offersToggles").onclick();
                        document.getElementById("PROAD").pause();
                    }, 8e3);
                }
    ...
    function drawWeaponDisplay(weapon, index, START_WEAPON) {
        ...
        document.getElementById("WEAPONdisplay").innerHTML = `
        <div style="font-size: 30px; color: ${tierColor(weapon)}; text-align: center; margin-top: 5px; width: 100%;">Lvl ${weapon.level} ${weapon.name}</div>
        <hr>
        <div id="weaponImage" style="position: absolute; top: 70px; left: 10px; width: 180px; height: 180px;"></div>
        <div style="position: absolute; top: 70px; left: 200px;">
        ${weapon.desc}
    ...
    function enterBarForWeapon(type, weapon) {
        ...
        if (type == "damage") {
            ...
            let indexAdjusted = (((weapon.dotDamage + Weapon.dotData.level[weapon.level]) / maxdmg) * maxwidth) / maxwidth;
            text = `
            <div style="position: relative; width: ${maxwidth}px;">
            DOT Damage: ${abbreviateNumber(weapon.dotDamage)}
            <div style="display: ${Weapon.dotData.level[weapon.level] ? "block" : "none"};position: absolute; top: 0px; right: 0px; color: #00ff00;">
            +${abbreviateNumber(Weapon.dotData.level[weapon.level])}
            </div>
            </div>
            <div style="position: relative; width: ${maxwidth}px; height: 3px; background-color: #fff;">
            <div style="position: absolute; top: 0px; left: 0px; height: 3px; width: ${indexAdjusted * 100}%; background-color: #02de02;">
    ...
    function customizeWeapon(weapon, shape, slot) {
        if (true) {
            ...
            document.getElementById("Upgrade").onclick = function () {
                ...
                document.getElementById("UPGRADE").onclick = function () {
                    if (sliverUpgradesByTier[weapon.tier].weapons[weapon.level]) {
            ...
            if (shapes.find(e => e.slot == i)) {
                ...
                if (newShape.baseShielding) {
                    newShape.shields.push({
                        maxhealth: newShape.baseShielding.health,
                        health: newShape.baseShielding.health,
        ...
        if (gameMode == 2) {
            ...
            for(let tt = 0; tt < 9; tt++) {
                for (let i = 0; i < players[0].robots.length; i++) {
                    ...
                    if (playerRobot) {
                        let possibleShape = shapeData.filter(e => e.tier == playerRobot.tier);
                        possibleShape = possibleShape[Math.floor(possibleShape.length * Math.random())];
                        let tmpShape = new shape(possibleShape, null, true);
                        for (let level = 0; level < playerRobot.level - 1; level++) {
                        ...
                        newShape.weapons = [];
                        let heavyWeapons = playerRobot.weapons.filter(e => e.type == "Heavy").sort((a, b) => b.level - a.level).sort((a, b) => b.tier - a.tier)[0];
                        if (playerRobot.weapons.filter(e => e.type == "Heavy").length == 0) {
                            if (playerRobot.weapons.filter(e => e.type == "Light").length) {
                                heavyWeapons = playerRobot.weapons.filter(e => e.type == "Light").sort((a, b) => b.level - a.level).sort((a, b) => b.tier - a.tier)[0];
                            }
                        }
                        if (heavyWeapons) {
                            let possibleWeapon = weaponData.filter(e => e.type == "Heavy" && e.tier == heavyWeapons.tier);
                            let index = heavyWeapons.tier - 1;
                            while (possibleWeapon.length == 0) {
                                possibleWeapon = weaponData.filter(e => e.type === "Heavy" && e.tier === index);
                                index--;
                            }
                            possibleWeapon = possibleWeapon[Math.floor(Math.random() * possibleWeapon.length)];
                            let tmpWeapon = new weapon(possibleWeapon);
                            for (let level = 0; level < heavyWeapons.level - 1; level++) {
                        ...
                        let lightWeapons = playerRobot.weapons.filter(e => e.type == "Light").sort((a, b) => b.level - a.level).sort((a, b) => b.tier - a.tier)[0];
                        if (playerRobot.weapons.filter(e => e.type == "Light").length == 0) {
                            if (playerRobot.weapons.filter(e => e.type == "Heavy").length) {
                                lightWeapons = playerRobot.weapons.filter(e => e.type == "Heavy").sort((a, b) => b.level - a.level).sort((a, b) => b.tier - a.tier)[0];
                            }
                        }
                        if (lightWeapons) {
                            let possibleWeapon = weaponData.filter(e => e.type == "Light" && e.tier == lightWeapons.tier);
                            let index = lightWeapons.tier - 1;
                            while (possibleWeapon.length == 0) {
                            ...
                            possibleWeapon = possibleWeapon[Math.floor(Math.random() * possibleWeapon.length)];
                            let tmpWeapon = new weapon(possibleWeapon);
                            for (let level = 0; level < lightWeapons.level - 1; level++) {
                                upgradeWeapon(tmpWeapon, null, null, true);
                            }
                        ...
                        let totalTiersModeule = 0;
                        for (let sad = 0; sad < playerRobot.modules.length; sad++) {
                            totalTiersModeule += playerRobot.modules[sad].tier;
                        }
                        let avgModuleLevel = 0;
                        for (let sad = 0; sad < playerRobot.modules.length; sad++) {
                            avgModuleLevel += playerRobot.modules[sad].level;
                        }
                        avgModuleLevel = Math.round(avgModuleLevel / playerRobot.modules.length);
                        if (isNaN(avgModuleLevel)) avgModuleLevel = 1;
                        totalTiersModeule = Math.min(4, Math.round(totalTiersModeule / playerRobot.modules.length));
                        if (isNaN(totalTiersModeule)) totalTiersModeule = null;
                        newShape.modules = [];
                        let healthIncrease = 1;
                        let dmgIncrease = 1;
                        dmgIncrease += (newShape.baseDamageIncrease || 0);
                        if (playerRobot.modules.length) {
                            let possibleModule = moduleData.filter(e => e.tier == totalTiersModeule);
                            let index = totalTiersModeule - 1;
                            while (possibleModule.length == 0) {
                                possibleModule = moduleData.filter(e => e.tier === index);
                                index--;
                            }
                            possibleModule = possibleModule[Math.floor(Math.random() * possibleModule.length)];
                            let tmpModule = new module(possibleModule);
                            for (let level = 0; level < avgModuleLevel - 1; level++) {
    ...
    function enterBarForShape(type, shape) {
        ...
        if (type == "health") {
            ...
            let indexAdjusted = (((shape.baseShielding.health + Shape.shieldData.level[shape.level]) / maxspeed) * maxwidth) / maxwidth;
            text = `
            <div style="position: relative; width: ${maxwidth}px;">
            Shield Durability: ${abbreviateNumber(shape.baseShielding.health)}
            <div style="display: ${Shape.shieldData.level[shape.level] ? "block" : "none"};position: absolute; top: 0px; right: 0px; color: #00ff00;">
            +${abbreviateNumber(Shape.shieldData.level[shape.level])}
            </div>
            </div>
            <div style="position: relative; width: ${maxwidth}px; height: 3px; background-color: #fff;">
            <div style="position: absolute; top: 0px; left: 0px; height: 3px; width: ${indexAdjusted * 100}%; background-color: #02de02;">
    ...
    function customizeShape(shape) {
        let moduleOn = false;
        document.getElementById("sideDisplay").innerHTML = `
        <div style="margin-top: 5px; color: ${tierColor(shape)}; width: 100%; text-align: center; font-size: 40px;">Lvl ${shape.level} ${shape.name}</div>
        <hr>
        ...
        document.getElementById("upgradeShape").onclick = function () {
            ...
            document.getElementById("UPGRADE").onclick = function () {
                if (sliverUpgradesByTier[shape.tier].shapes[shape.level]) {
    ...
    function drawShapeEquip(shape, currentIndex, oldShape) {
        ...
        document.getElementById("WEAPONdisplay").innerHTML = `
        <div style="font-size: 30px; text-align: center; margin-top: 5px; color: ${tierColor(shape)}; width: 100%;">Lvl ${shape.level} ${shape.name}</div>
        <hr>
        <div id="weaponImage" style="position: absolute; top: 70px; left: 10px; width: 180px; height: 180px;"></div>
        <div style="position: absolute; top: 70px; left: 200px;">
        ${shape.desc}
            ...
            if (i == rewardIndex) {
                ...
                if (element) {
                    ...
                    if (last6Chars == "weapon") {
                        let findIndexId = element.id.split("");
                        let index = "";
                        for (let i = 0; i < findIndexId.length; i++) {
                            if (!isNaN(element.id[i])) {
                                index += findIndexId[i];
                            } else {
                                break;
                            }
    ...
    function tierColor(obj) {
        return obj.tier == 3 ? "#fcdf00" : obj.tier == 2 ? "#7702bf" : obj.tier == 1 ? "#38b0f5" : "#d3d3d3";
    }
    ...
    function endGame(Player, rewardsMulti) {
        ...
        SLIVER += Math.max(Math.round(Player.dmg / 500e3), 0) * 250e3;
    ...
    function updatePlayer(delta) {
        ...
        if (Player.robotIndex >= 0) {
            ...
            for (let i = 0; i < robot.shields.length; i++) {
                ...
                <div style="position: absolute; top: 0px; left: 0px; width: ${100 * (shield.health / shield.maxhealth)}%; height: 100%; background-color: #02ccaa;">
                </div>
                <div style="position: absolute; top: 0px; left: 5px; font-size: 17px;">
                ...
                if (shield.regen) {
                    ...
                    if (Date.now() - shield.lastRegen >= 5e3) {
                ...
                if (shield.health > 0) {
                    let touchBullets = projectiles.filter(e => !e.isAlly && Math.hypot(e.y - robot.y, e.x - robot.x) >= e.scale - 180 && Math.hypot(e.y - robot.y, e.x - robot.x) <= e.scale + robot.scale);
                    for (let t = 0; t < touchBullets.length; t++) {
                        ...
                        changeHealth(shield, -touchBullets[t].dmg, false, touchBullets[t].owner);
            ...
            for (let tt = 0; tt < robot.effects.length; tt++) {
                ...
                if (effect) {
                    ...
                    if (effect.name == "cold pulse") {
                        if(effectNames["cold pulse"] == null) {
                            effectNames["cold pulse"] = {
                                amount: 1,
                                duration: effect.lastTime,
                                orginal: effect.oldLastTime
                            };
                        }else {
                            effectNames["cold pulse"].amount++;
                            effectNames["cold pulse"].duration = effect.lastTime;
                        }
                        ...
                        if(effectNames["dot"] == null) {
                            ...
                            effectNames["dot"].amount++;
                            effectNames["dot"].duration = effect.lastTime;
                        }
                        if (effect.dmgOverTime == null) effect.dmgOverTime = 0;
                        effect.dmgOverTime -= delta;
                        if (effect.dmgOverTime <= 0) {
                            if(robot.turnDmgIntoDOT) {
                                robot.grayDamage += Math.abs(effect.dmg * .4);
            ...
            for (let t = 0; t < touchBullets.length; t++) {
                if (touchBullets[t].dmg > 0 && touchBullets[t].dotEffect) {
                    robot.effects.push({
            ...
            for(let i in effectNames) {
                ...
                <div style="position: absolute; top: 0px; left: 5px; height: 40px; width: 40px; background-size: 40px 40px; background-image: url('${i == "dot" ? "./images/viper.png" : "./images/cold_pulse.png"}');">
                </div>
            ...
            if (robot.ability && !robot.isFREEZE) {
    ...
    function changeHealth(object, amount, isPlayer, doer, noDefense) {
        try {
            ...
            if (!object.turnDmgIntoDOT && amount < 0) {
                object.grayDamage += Math.abs(amount * .4);
            }
            if (amount != 0) {
                if (amount < 0 && object.reflector && !object.turnDmgIntoDOT) {
            ...
            if (enemy && playerRobot) {
                ...
                if (!enemy.isFREEZE) {
                    if (enemy.name == "Burner Circle") {
                        ...
                        if (enemy.lastFire == null) enemy.lastFire = 0;
                        if (Date.now() - enemy.lastFire >= 100) {
                            enemy.lastFire = Date.now();
                            projectiles.push({
                                x: enemy.x,
                                y: enemy.y,
                    ...
                    } else if (enemy.name == "Level 5 Boss") {
                        let percent = (enemy.health / enemy.maxhealth) * 100;
                        let angle = Math.atan2(playerRobot.y - enemy.y, playerRobot.x - enemy.x);
                        let distance = Math.hypot(playerRobot.y - enemy.y, playerRobot.x - enemy.x);
                        ...
                        if (enemy.lastFire == null) enemy.lastFire = 0;
                        if (Date.now() - enemy.lastFire >= 200) {
                            enemy.lastFire = Date.now();
                            projectiles.push({
                                x: enemy.x,
                                y: enemy.y,
                        ...
                        if (Date.now() - enemy.lastSpawnEnemy >= (percent <= 10 ? 1e3 : 5e3)) {
    ...
    function updateBullets(delta) {
        for (let i = 0; i < projectiles.length; i++) {
            ...
            if (bullet) {
                let asd = buildings.find(e => circleSquareCollision(bullet, e));
        ...
        for (let i = 0; i < enemies.length; i++) {
            ...
            if (enemy) {
                ...
                if (enemy.name == "Burner Circle") {
                    ...
                    renderCircle(0, 0, enemy.scale, ctx, false, false, Date.now() - enemy.damagedTime <= 20 ? "#fff" : "#f00", enemy.isFREEZE ? "#fff" : "#000");
                }
                if (enemy.isBoss) {
                    ctx.fillStyle = "#fff";
    ...
    function PLAYERKNOCKBACK(robot) {
        for (let i = 0; i < players.length; i++) {
            ...
            if (ROBOT) {
                ...
                if(distance <= robot.scale + ROBOT.scale) {
                    let angle = Math.atan2(ROBOT.y - robot.y, ROBOT.x - robot.x) + Math.PI;
                    let tmpInt = distance - (robot.scale + ROBOT.scale);
                    tmpInt = (tmpInt * -1) / 2;
            ...
            if (robot) {
                    ...
                    if (effect) {
                        ...
                        if (effect.name == "cold pulse") {
                            ...
                            if (effect.dmgOverTime <= 0) {
                                ...
                                changeHealth(robot, -effect.dmg, effect.owner.isMe ? true : false, null, true);
                                effect.dmgOverTime = 1000;
                            }
                        }
                        if (effect.lastTime <= 0) {
                ...
                if(robot.isFREEZE) {
                    ...
                    for(let oioi = 0; oioi < players.length; oioi++) {
                        ...
                        if(ROBOTBO && players[oioi].isAlly != players[i].isAlly) {
                ...
                let touchBullets = projectiles.filter(e => e.isAlly != players[i].isAlly && Math.hypot(e.y - robot.y, e.x - robot.x) <= e.scale + robot.scale);
                for (let t = 0; t < touchBullets.length; t++) {
                    if (touchBullets[t].dmg > 0 && touchBullets[t].dotEffect) {
                        robot.effects.push({
                            name: touchBullets[t].dotEffect.name,
                ...
                if (robot.ability && !robot.isFREEZE) {
                    if(robot.ability.name == "Self Heal") {
                        ...
                        let lowestRange = robot.weapons.sort((a, b) => a.range - b.range)[0];
                        if(Math.hypot(robot.y - robot.target.y, robot.x - robot.target.x) <= lowestRange.range && Math.hypot(robot.y - robot.target.y, robot.x - robot.target.x) <= 1200 + robot.target.scale && robot.ability.name == "Cold Pulse") {
                            robot.doUseAbility = true;
                        }else {
                            robot.doUseAbility = false;
                        }
