(function () {
   ...
   - Bug Fixes<br><br>
    ...
    var shapeData = [{
        ...
        healthData: {
            base: 108e3,
            level: [0, 11e3, 11e3, 13e3, 13e3, 13e3, 17e3, 17e3, 17e3, 20e3, 22e3, 25e3],
        },
        ability: {
            ...
            dotData: {
                base: 5e3,
                level: [0, 300, 300, 300, 500, 500, 700, 700, 1e3, 1e3, 1500, 2e3]
            },
            charges: 7,
            lastingTime: 500,
            reload: 3e3
    ...
    var weaponData = [{
        ...
        dotData: {
            base: 80,
            level: [0, 60, 60, 60, 60, 60, 60, 60, 60, 60, 60, 80]
        },
    ...
    var moduleData = [{
        ...
        `,
        imageSource: "./images/modules/god_complex_equation.png",
        healthIncreaseData: {
            base: .05,
            level: [0, .05, .05, .05, .05, .05]
        },
        grayDamageDecreaseData: {
            base: 0.012,
            level: [0, 0.012, 0.025, 0.025, 0.05, 0.05]
        },
        dmgIncreaseData: {
            base: 0.02,
            level: [0, 0.02, 0.02, 0.02, 0.02, 0.02]
    ...
    function generateRandomName() {
        ...
        return listOfNames[Math.floor(Math.random() * listOfNames.length)] + (p2w ? "_p2w" : "");
    }
    var KILLLOGS = [];
    ...
    function setWeaponData(robot, weapons) {
        ...
        for (let t = 0; t < weapons.length; t++) {
            ...
            if (!newWeapon.baseShielding) robot.weapons.push(newWeapon);
        }
    }
    ...
    function setUpBotData(orginalRobot, weapons, modules, microchips, isAlly, gameMode, spawnLocations) {
    ...
    function setUpPlayersData(gameMode, spawnLocations) {
        ...
        if (hasPlayers()) {
            ...
            let amount = (gameMode == 5 ? 1 : gameMode == 3 ? 10 : gameMode == 4 ? 19 : 11);
            for (let i = 0; i < amount; i++) {
                let isAlly = gameMode == 5 ? false : gameMode == 3 ? false : i < 5 ? true : false;
                let name = generateRandomName();
                players[1 + i] = {//"Bluebell"
            ...
            for (let tt = 0; tt < amount; tt++) {
                ...
                let robotAmount = (players[0].robots.length * (gameMode == 3 ? .75 : 1));
                for (let i = 0; i < robotAmount; i++) {
                    if (players[tt + 1].name == "Bluebell") {
                        ...
                        players[1 + tt].robots.push(setUpBotData(orginalRobot, weapons, modules, microchips, isAlly, gameMode, spawnLocations));
        ...
        if (shape.ability) {
            if (shape.ability.shieldHp) {
                ...
                if (shape.level < 12) {
                    ...
                    shape.ability.shieldHp *= shape.level == 13 ? 1.5 : 1.2;
                }
            }
            ...
            if (shape.ability.dmg) {
                ...
                if (shape.level < 12) {
                    ...
                    shape.ability.dmg *= shape.level == 13 ? 1.5 : 1.2;
                }
            }
            if (shape.ability.dotDamage) {
                ...
                if (shape.level < 12) {
                    ...
                    shape.ability.dotDamage *= shape.level == 13 ? 1.5 : 1.2;
                }
            }
        }
    ...
    function obtainBattleRewards(result, oldYAY, rewardsMulti) {
        ...
        total.oxp += Math.max(Math.round(result.dmg / 1e6), 0) * 15;
    ...
    function checkIfHitByProj(robot, isAlly) {
        ...
        for (let t = 0; t < touchBullets.length; t++) {
            ...
            if (bullet.dmg > 0) {
                ...
                if (bullet.rustEffect) {//dotEffect
                ...
                if (bullet.dotEffect) {
                    ...
                    if (bullet.dotEffect.effectID ? robot.effects.filter(e => e.name == "dot" && e.effectID == bullet.dotEffect.effectID).length < 60 : true) {
                        robot.effects.push({
                            neg: true,
            ...
            if (weapon.continuousReload) {
                ...
                if (weapon.ammo < weapon.maxammo) {
                    ...
                    if (Date.now() - weapon.lastChange >= (weapon.reload * robot.reloadMoveMulti)) {
                        ...
                        if (weapon.name == "Pantagruel" || weapon.name == "Gangantua") {
    ...
    function doOtherBotStuff(robot, isAlly) {
        ...
        if (robot.ability) {
            ...
            if (["Reinforce Hull", "Cold Pulse", "Divine Judgement", "Full Action", "Stealth", "Domain Expansion: Infinite Void", "Paladin", "Dragon Flight", "Phase Shift", "Ultimate Mending", "Self Heal", "Retribution"].includes(abilityName) && Date.now() - robot.damagedTime <= 50) {
    ...
    function updateRobot(robot, isAlly, delta, index) {
        ...
        if (robot.ability && robot.isMe) {//abilityCharges
        ...
        if (robot.health <= 0 || robot.kill) {
            if (robot.revive && !robot.kill) {
                ...
                if (robot.isMe) {
                    ...
                    let hasRobots = players[0].robots.find(e => !e.USED);
                    document.getElementById("chooseSlot").style.bottom = `0px`;
                    document.getElementById("mapName").style.display = "block";
                    if (!hasRobots) {
                        document.getElementById("chooseSlot").style.bottom = `-200px`;
                        document.getElementById("mapName").style.display = "none";
                        endGame([...players], (hasPlayers() ? .5 : null));
                    }
                    updateChooseSlots();
                } else {
                    ...
                    if (!players[index].robots[players[index].robotIndex]) {
                        ...
                        if (haveEnemyes) {
                            ...
                            document.getElementById("healthBar").style.display = "none";
                            document.getElementById("chooseSlot").style.bottom = "-200px";
                            endGame([...players], (player.gameMode == 3 || player.gameMode == 4 ? 15 : 3));
                        }
                    }
                }
            }
    ...
    function changeHealth(object, { amount, graydmg, onDamageHealBack, isDotDamage, defensePointsBypass, bypassReflector, damageTypeName }, isPlayer, doer, noDefense) {
        try {
            ...
            if (amount != 0) {
                    ...
                    if (doer && doer.isMe) {
                        if (amount < 0) {
                            if (isDotDamage) {
                                ...
                                addText(object.x, object.y, amount, 1000, defensePointsBypass + 1 >= 1 ? "#ffff00" : "#fff");
                            }
                        } else {
                ...
                if (amount < 0 && object.effects && !object.effects.find(e => e.name == "cooldown timer" && e.onDamageHealShield == "Ultimate Green Circle") && object.name == "Ultimate Green Circle") {
    ...
    function antiTier4_5(bullet, enemy) {
        ...
        if (enemy.maxhealth >= 350e3) {
            ...
            amount += Math.min(Math.floor(mm / 25e3) * .1, 2.5);
        }
                    ...
                    if (enemy.name == "Burner Circle") {
                        ...
                        if (Date.now() - enemy.lastFire >= 50) {
                            ...
                            if (enemy.name == "Level 10 Boss") {
                                dmg *= Math.max(((1 - (enemy.health / enemy.maxhealth)) * 6), 1);
                            }
                            projectiles.push({
                                x: enemy.x,
                                y: enemy.y,
                ...
                for (let t = 0; t < touchBullets.length; t++) {
                    ...
                    if (bullet.dmg > 0) {
                        ...
                        if (bullet.dotEffect) {
                            ...
                            if (bullet.dotEffect.effectID ? enemy.effects.filter(e => e.name == "dot" && e.effectID == bullet.dotEffect.effectID).length < 60 : true) {
                                enemy.effects.push({
                                    neg: true,
                                    resetDOT: bullet.dotEffect.resetDOT,
                                    name: bullet.dotEffect.name,
    ...
    function doMoveStuff(robot, isAlly) {
        ...
        let TARGET = null;
        if ((player.gameMode == 3 || player.gameMode == 4 ? true : !nearestBeacon) && Math.hypot(nearestEnemy.y - robot.y, nearestEnemy.x - robot.x) >= nearestEnemy.scale + robot.scale + 20) {
            TARGET = nearestEnemy;
            ...
            if (domain) {
                ...
                if (domain.name == "Roulette Strike") {
                    ...
                    if (domain.effectThingy <= 0) {
                        cando = true;
                        domain.effectThingy = 500;
                    }
                    for (let t = 0; t < players.length; t++) {
                        let robot = players[t].robots[players[t].robotIndex];
                        if (robot && players[t].isAlly != domain.isAlly && cando) {
                            if (Math.hypot(domain.y - robot.y, domain.x - robot.x) <= domain.scale + (robot.scale * 1.8)) {
                    ...
                    for (let t = 0; t < enemies.length; t++) {
                        let enemy = enemies[t];
                        if (cando && Math.hypot(domain.y - enemy.y, domain.x - enemy.x) <= domain.scale + (enemy.scale * 1.8)) {
