   ...
   - Bug Fixes<br><br>
    ...
    function leagueToEquipment(type, P2W, METAROBOT) {
        ...
        if (player.league >= 7e3) {
            if(Math.random() < .25 && (type == "heavy" || type == "light")) {
                tier = 5;
        ...
        if (gameMode == 2) {
            for (let i = 0; i < 11; i++) {
                players[1 + i] = {
                    ...
                    isAlly: i < 5 ? true : false,
                    notMe: true,
                    robotIndex: 0,
                    robots: []
                };
            }
            for (let tt = 0; tt < 11; tt++) {
        ...
        try {
            ...
            if (amount != 0) {
                ...
                if (object.turnDmgIntoDOT && amount < 0 && doer && !noDefense) {
                    ...
                    if(object.effects.filter(e => e.name == "defense points").length < 250 && object.effects.find(e => e.name == "stampede")) {
                        object.effects.push({
                            name: "defense points",
                            amount: 2,
                            lastTime: 20e3
    ...
    function updateGame() {
        ...
        if (player.gameMode >= 0) {
            ...
            if (gameTimePlayed <= 60e3) {
                document.getElementById("timePlayed").innerHTML = `${player.gameMode == 0 ? `Wave: ${extermination.waveIndex+1} | ` : ""}${fpsInfo.real} | 0:${gameTimePlayed <= 10e3 ? "0" : ""}${removeDecimals(gameTimePlayed / 1000)}`;
            } else {
                let value = (gameTimePlayed / 1000) / 60;
                value = value.toString().split(".");
                let minutes = value[0];
                let seconds = parseFloat("0." + value[1]) * 60;
                document.getElementById("timePlayed").innerHTML = `${player.gameMode == 0 ? `Wave: ${extermination.waveIndex+1} | ` : ""}${fpsInfo.real} | ${minutes}:${seconds <= 10 ? "0" : ""}${removeDecimals(seconds)}`;
            }
