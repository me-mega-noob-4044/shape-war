(function () {
   ...
   - Bug Fixes<br><br>
    ...
    var weaponData = [{
        ...
        damageData: {
            base: 1692,
            level: [0, 133, 133, 133, 133, 133, 133, 133, 133, 133, 133, 133]
        },
        ...
        damageData: {
            base: 2538,
            level: [0, 200, 200, 200, 200, 200, 200, 200, 200, 200, 200, 200]
        },
    ...
    function generateRandomName() {
        let isP2W = Math.random() < (player.league >= 51e2 ? 0.3 : 0.05);
        let name = "";
    ...
    function addGrayDamage(shape, amount, graydmg) {
        ...
        shape.grayDamage += Math.abs(amount * (graydmg ? 1 : shape.effects && shape.effects.find(e => e.name == "stampede") ? .05 : shape.name == "Orange Circle" ? .25 : .4) * grayDamageMulti);
    }
    ...
    function updatePlayer(delta) {
        ...
        if (Player.robotIndex >= 0) {
            ...
            if(robot.ability) {
                ...
                document.getElementById("abilityCharges").style.left = robot.weapons.length > 4 ? robot.effects.length ? `165px` : "20px" : "350px";
    ...
    function checkIfHit(proj, enemy, plus) {
        ...
        let tmplocation = {
            x: proj.x + Math.cos(proj.dir) * d,
            y: proj.y + Math.sin(proj.dir) * d
        };
        if(Math.hypot(tmplocation.y - enemy.y, tmplocation.x - enemy.x) <= (proj.scale * (plus == "shield" ? 1.5 : 1)) + enemy.scale + (plus == "shield" ? 20 : 0)) {
            return true;
        }
        return false;
    }
        ...
        for (let i = 1; i < players.length; i++) {
            ...
            if (robot) {
                ...
                for (let t = 0; t < touchBullets.length; t++) {
                    if (touchBullets[t].dmg > 0) {
                        if(touchBullets[t].knockback) {
                            robot.velx += Math.cos(touchBullets[t].dir) * touchBullets[t].knockback;
                            robot.vely += Math.sin(touchBullets[t].dir) * touchBullets[t].knockback;
                        }
                        if (touchBullets[t].dotEffect) {
                            robot.effects.push({
                                name: touchBullets[t].dotEffect.name,
                    ...
                    let hadHealth = robot.health > 0;
                    if (!touchBullets[t].aoeRange) changeHealth(robot, {
                        amount: -touchBullets[t].dmg,
                        graydmg: touchBullets[t].grayDamageAmount,
                        defensePointsBypass: touchBullets[t].defensePointsBypass,
                        bypassReflector: touchBullets[t].bypassReflector
                    }, true, touchBullets[t].owner);
                    if (touchBullets[t].aoeRange != null) {
                        for (let asdasd = 0; asdasd < players.length; asdasd++) {
                            let ROBOBT = players[asdasd].robots[players[asdasd].robotIndex];
    ...
    function updateGame() {
        ...
        if (player.gameMode >= 0) {
            ...
            if (gameTimePlayed <= 60e3) {
                document.getElementById("timePlayed").innerHTML = `${fpsInfo.real} | 0:${gameTimePlayed <= 10e3 ? "0" : ""}${removeDecimals(gameTimePlayed / 1000)}`;
            } else {
                let value = (gameTimePlayed / 1000) / 60;
                value = value.toString().split(".");
                let minutes = value[0];
                let seconds = parseFloat("0." + value[1]) * 60;
                document.getElementById("timePlayed").innerHTML = `${fpsInfo.real} | ${minutes}:${seconds <= 10 ? "0" : ""}${removeDecimals(seconds)}`;
            }
