(function () {
   ...
   - Bug Fixes<br><br>
    ...
    function makeNewSpecialAddiction(name, {health, reflector}, type, specialOf, source) {
        if (type == "shape") {
            ...
            newshape.desc = `
            Limited Edition Shape with increased stat bonuses.
            <br><br>
            ` + newshape.desc;
            ...
            if (newshape.reflector) newshape.reflector = reflector;
...
*/
    ...
    function doMicrochipStuff(element, button, shape) {
        ...
        `;
        if(shape.specialOf) {//width
            document.getElementById(`shapeImage2`).style.backgroundSize = `375px 375px`;
    ...
    function drawShapeEquip(shapes, currentIndex, oldShape, slot, HA, inInventory) {
        ...
        console.log(shape, shape.specialOf);
        if(shape.specialOf) {//width
            document.getElementById(`shapeImage`).style.backgroundSize = `375px 375px`;
            document.getElementById(`shapeImage`).style.backgroundImage = `url('${shape.specialOf}')`;
        }else {
    ...
    function doPlayerMovement(robot, delta, { hasStampede, slowdownMulti }) {
            ...
            if (shield) {
                ...
                if (shield.health > 0 || shield.type == "purple") {
                    ...
                    for (let t = 0; t < touchBullets.length; t++) {
                        ...
                        }, "LMAOLMAOLMAOLMAO", touchBullets[t].owner);
    ...
    function checkIfHitByProj(robot) {
    ...
    function doEffectStuff(robot, delta) {
        robot.dotDamage = 0;
        let effectNames = {};
    ...
    function doWeaponStuff(robot, delta) {
    ...
    function collideWithBuildings(robot, delta) {
    ...
    function doAbilityFunction(robot, delta) {
        if (robot.ability.name == "Divine Judgement") {
            deltaTimer -= delta;
            if (deltaTimer <= 0) {
                deltaTimer = 50;
        ...
        if (robot.ability.name == "Retribution" || robot.ability.name == "Paladin") {
            if (robot.ability.name == "Paladin") {
                if (robot.shields[0] && robot.shields[0].health > 0) {
                    ...
                    changeHealth(robot, {
                        amount: increase * 5
                    }, robot.isMe ? true : "LMAOLMAOLMAOLMAO");
                }
            }
    ...
    function updatePlayer(delta) {
        ...
        if (Player.robotIndex >= 0) {
            ...
            let { effectNames, lastStand, slowdownMulti } = doEffectStuff(robot, delta);
            document.getElementById("LMAOLMAOLMAOLMAO").innerHTML = "";
            ...
            drawShieldBars(robot);
            checkIfHitByProj(robot);
            ...
            `;
            drawWeaponUI(robot, delta);
            doWeaponStuff(robot, delta);
            ...
            collideWithBuildings(robot, delta);
            if (robot.health <= 0) {
                if (robot.revive) {
    ...
    function changeHealth(object, { amount, graydmg, defensePointsBypass, bypassReflector, REFLECTOOOOR }, isPlayer, doer, noDefense) {
        try {
            ...
            if (isPlayer != "LMAOLMAOLMAOLMAO" && isPlayer != "HEAL AHAHAHAHHA" && amount != 0) {
                ...
                if (amount < 0) {
                    ...
                    if(playerRobot) {
                        let amps = playerRobot.effects.filter(e => e.name == "nuclear amps");
                        if(amps.length < 95) {
                            if(playerRobot.dmgDamageDone == null) playerRobot.dmgDamageDone = 0;
                            playerRobot.dmgDamageDone += Math.abs(amount);
                            if(playerRobot.dmgDamageDone - 40e3 >= 0) {
                                let Amps = playerRobot.modules.filter(e => e.name == "Nuclear Amplifier");
                                if(amps.length + Amps.length >= 95) {
                                    let amount = 95 - amps.length;
                                    console.log(amount, amps.length);
                                    let healAmount = 0;
                                    Amps.forEach(e => {
    ...
    function drawCanvasHealthBars(robot, isAlly, ctx) {
        ...
        if (!robot.invis && !isAlly) {
            ...
            ctx.fillText(abbreviateNumber(removeDecimals(Math.hypot(robot.y - playerRobot.y, robot.x - playerRobot.x))) + " PX", 0, 0);
            ctx.restore();
        }
                        ...
                        if (player && players[t].isAlly == players[i].isAlly) {
                            if (Math.hypot(robot.y - player.y, robot.x - player.x) <= 300 + player.scale) {
                                changeHealth(player, {
                                    amount: -25e3,
                                    graydmg: 0
                                }, theowner.isMe ? true : "LMAOLMAOLMAOLMAO", undefined);
                            }
                        }
                    }
                    bombeffect.push({
                ...
                for (let tt = 0; tt < robot.shields.length; tt++) {
                    ...
                    if (shield.health > 0 || shield.type == "purple") {
                        ...
                        for (let t = 0; t < touchBullets.length; t++) {
                            ...
                            changeHealth(shield, {
                                amount: (touchBullets[t].dmg * (shield.type == "purple" ? 1 : -1))
                            }, touchBullets[t].owner ? touchBullets[t].owner.isMe ? false : "LMAOLMAOLMAOLMAO" : "LMAOLMAOLMAOLMAO", touchBullets[t].owner);
                            touchBullets[t].dmg = 0;
                            touchBullets[t].kill = true;
                        }
                    }
