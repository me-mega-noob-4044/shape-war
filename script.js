(function () {
    ...
    /*var backgroundMusic = {
        audio: new Audio("https://cdn.discordapp.com/attachments/836319356186263552/1152071310864814222/TheFatRat_Unity.mp3"),
        status: false
    };
    function startMusic() {
        if(!backgroundMusic.status) {
            backgroundMusic.status = true;
            backgroundMusic.audio.loop = true; // Set loop property to true to enable looping
    ...
    var shapeData = [{
        ...
        Shape comes with built-in arc reactor, increasing its damage by 35%.<br><br>
    ...
    var weaponData = [{
        ...
        damageData: {
            base: 390,
            level: [0, 50, 50, 55, 70, 70, 80, 80, 90, 100, 150, 150],
        },
        dotData: {
            base: 30,
        ...
        damageData: {
            base: 930,
            level: [0, 120, 130, 150, 166, 177, 211, 231, 251, 271, 271, 272],
        },
        ...
        if (gameMode == 2) {
            ...
            for (let i = 0; i < players[0].robots.length; i++) {
                ...
                if (playerRobot) {
                    ...
                    let dmgIncrease = 1;
                    dmgIncrease += (newShape.baseDamageIncrease || 0);
                    if (totalTiersModeule >= 0) {
                        let Module = playerRobot.modules.sort((a, b) => b.level - a.level)[0];
                        if (Module) {
                            let possibleModule = moduleData.filter(e => e.type == Module.type && e.tier == Module.tier);
                            let index = Module.tier - 1;
                            while (possibleModule.length == 0) {
                                possibleModule = moduleData.filter(e => e.type === Module.type && e.tier === index);
                                index--;
                            }
                    ...
                    players[1].robots.push(newShape);
                } else {
                    players[1].robots.push(null);
                }
            }
        }
        updateChooseSlots();
    ...
    function fireWeapon(weapon, shape, isAlly) {
        ...
        if (!isAlly) {
            if (shape.target) {
                shape.dir = Math.atan2(shape.target.y - shape.x, shape.target.x - shape.x);
            }
        }
        if (hardpoints == 1) {
    ...
    function endGame(Player, rewardsMulti) {
        ...
        let SLIVER = 0;
        let GOLD = 0;
        SLIVER += (5e3 * Player.kills);
        SLIVER += (75 * (Math.abs(Player.dmg) / 1000));
        SLIVER += Math.max(Math.round(Player.dmg / 1e6), 0) * 100e3;
                ...
                for (let tt = 0; tt < enemy.effects.length; tt++) {
                    ...
                    if (effect) {
                        ...
                        if (effect.name == "cold pulse") {
                            ...
                            effect.dmgOverTime -= delta;
                            if (effect.dmgOverTime <= 0) {
                                changeHealth(enemy, -effect.dmg, true);
                                effect.dmgOverTime = 1000;
                            }
                        }
                        if (effect.lastTime <= 0) {
    ...
    function updatePlayers(delta) {
        for (let i = 1; i < players.length; i++) {
            ...
            if (robot) {
                ...
                for (let tt = 0; tt < robot.effects.length; tt++) {
                    ...
                    if (effect) {
                        ...
                        if (effect.name == "cold pulse") {
                            ...
                            effect.dmgOverTime -= delta;
                            if (effect.dmgOverTime <= 0) {
                                changeHealth(robot, -effect.dmg, true);
                                effect.dmgOverTime = 1000;
                            }
                        }
                        if (effect.lastTime <= 0) {
                ...
                if (!robot.isFREEZE) {
                    let nearestBeacon = buildings.filter(e => e.name == "beacon" && e.ENEMYPOINTS < 5e3).sort((a, b) => Math.hypot(a.y - robot.y, a.x - robot.x) - Math.hypot(b.y - robot.y, b.x - robot.x))[0];
                    let moveDir = null;
                    if(nearestBeacon && Math.hypot(nearestBeacon.y - robot.y, nearestBeacon.x - robot.x) >= 400 - robot.scale*2) {
                        moveDir = Math.atan2(nearestBeacon.y - robot.y, nearestBeacon.x - robot.x);
                    }else {
                        moveDir = null;
                    }
                    if(moveDir != null) {
                ...
                if(robot.isFREEZE) {
                    ...
                    robot.target = players[0].robots[players[0].robotIndex];
                    if(robot.target) {
                ...
                if (robot.ability && !robot.isFREEZE) {
                    ...
                    if (robot.abilityLast && robot.abilityLast > 0) {
                        ...
                        if (robot.ability.name == "Self Heal") {
                            ...
                            if (deltaTimer <= 0) {
                                let amount = robot.maxhealth * .025;
                                changeHealth(robot, Math.floor(amount), false);
    ...
    function updateGame() {
        ...
        if (player.gameMode >= 0) {
            ...
            for (let i = 0; i < allElements.length; i++) {
                ...
                if (element.id) {
                    if (element.id.indexOf("tracer") !== -1) {
