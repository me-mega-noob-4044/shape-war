(function () {
    ...
    - New Weapons: Blaze and Ember<br><br>
            ...
            if (robot) {
                ...
                if (robot.ability && !robot.isFREEZE) {
                    if (robot.ability.name == "Self Heal") {
                        ...
                        let lowestRange = robot.weapons.sort((a, b) => a.range - b.range)[0];
                        if (Math.hypot(robot.y - robot.target.y, robot.x - robot.target.x) <= lowestRange.range && Math.hypot(robot.y - robot.target.y, robot.x - robot.target.x) <= (robot.ability.name == "Retribution" ? 800 : 1200) + robot.target.scale && robot.ability.name == "Cold Pulse") {
                            robot.doUseAbility = true;
                        } else {
                            robot.doUseAbility = false;
                        }
