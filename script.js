(function () {
   ...
   - Bug Fixes<br><br>
    ...
    var weaponData = [{
        ...
        damageData: {
            base: 940,
            level: [0, 93, 93, 93, 93, 93, 93, 93, 93, 93, 93, 93]
        },
        ...
        damageData: {
            base: 1410,
            level: [0, 110, 110, 110, 110, 110, 110, 110, 110, 110, 110, 110]
        },
        ...
        damageData: {
            base: 108,
            level: [0, 12, 12, 18, 18, 24, 24, 24, 24, 36, 36, 48]
        },
...
*/
    ...
    function drawStoreDisplay(item, type) {
        ...
        if (type == "shapes") {
            ...
            ${item.healingMulti ? `Healing Effectiveness: ${removeDecimals(item.healingMulti*100)}%<br>` : ""}
            ${item.revive ? `Revival Amount: ${removeDecimals(item.revive * 100)}%<br>` : ""}
            `;
    ...
    function drawWeaponDisplay(weapons, index, HA, shape, slot, inInventory) {
        ...
        ${weapon.dotDamage ? `DOT Damage: ${abbreviateNumber(weapon.dotDamage)}<br>` : ""}        <p></p>
        </div>
        </div>
        </div>
    ...
    function customizeWeapon(weapon, shape, slot) {
        if (true) {
            ...
            document.getElementById("inventoryButton").onclick = function() {
                ...
                currentIndex = 0;
                inInventory = true;
                drawWeaponDisplay(inInventory ? filteredWeapons : weaponData.sort((a, b) => a.tier - b.tier), currentIndex, false, shape, slot, inInventory);
            };
            ...
            if (!(inInventory ? filteredWeapons[currentIndex - 1] : weaponData[currentIndex - 1])) {
                document.getElementById("goToPre").style.display = "none";
            }
        ...
        document.getElementById("Upgrade").onclick = function () {
            ...
            document.getElementById("UPGRADE").onclick = function () {
                if (sliverUpgradesByTier[weapon.tier].modules[weapon.level] && player.sliver - sliverUpgradesByTier[weapon.tier].modules[weapon.level] >= 0) {
                    ...
                    upgradeModule(weapon, currentIndex, weapons, false, false, shape, slot, inInventory);
                }
            }
        }
        document.getElementById("backButton").onclick = function() {
    ...
    function customizeModule(module, shape, slot) {
        if (true) {
            ...
            drawModuleDisplay(filteredWeapons, 0, false, shape, slot);
        ...
        for (let i = 0; i < 8; i++) {
            ...
            if (robot) {
                if(robot.USED) {
                    ...
                    <div style="z-index: 2; position: absolute; top: 0px; left: 0px; width: 100%; height: 100%; background-size: 100% 100%; background-image: url('./images/cant_use_shape.png');">
                    </div>
                    `;
                }
                                ...
                                if (player && players[t].isAlly != players[0].isAlly) {
                                    if (!player.effects.find(e => e.name == "phase shift") && Math.hypot(player.y - robot.y, player.x - robot.x) <= 1200 + player.scale) {
                                        ...
                                        if(player.ability) {
                                            ...
                                            if(player.ability.name == "Paladin" || player.ability.name == "Retribution") {
                                                player.shields = [];
                                            }
                                        }
                                    }
            ...
            for (let i = 0; i < robot.weapons.length; i++) {
                ...
                if (weapon.continuousReload) {
                    ...
                    if (weapon.name == "Atomizer" || weapon.name == "Nucleon") {
                        ...
                        if (keysPressed[32]) {
                            weapon.firedTime = Math.min(weapon.firedTime + delta, 15e2);
                        } else {
                    ...
                    if (weapon.canFire) {
                        if (weapon.lastFire == null) weapon.lastFire = 0;
                        robot.reloadMoveMulti = 1;
                        let fireRateMulti = (weapon.name == "Evora" || weapon.name == "Veyron" ? (weapon.firedTime/1000 == 1.5 ? 0.4 : 1) : (weapon.firedTime / 16000));
    ...
    function antiTier4_5(bullet, enemy) {
        ...
        if(enemy.maxhealth >= 300e3) {
            let mm = enemy.maxhealth - 300e3;
            amount += Math.min(Math.floor(mm/10e3)*.4, 25);
        }
        ...
        }else if(!bullet.weaponOwner.name.includes("Divine")) {
            ...
            if (robot) {
                        ...
                        if(effect.lastTime > 0) {
                            if (effect.name == "cold pulse") {
                                ...
                                if (effect.dmgOverTime <= 0) {
                                    ...
                                    changeHealth(robot, {
                                        amount: -effect.dmg
                                    }, effect.owner ? effect.owner : "LMAOLMAOLMAOLMAO", null, true);
                                    if(robot.health <= 0 && hadHealth) {
                                        addLogger("unknown bot", {
                                            name: "DOT EFFECT",
                                            level: 1
                ...
                for (let yyy = 0; yyy < robot.weapons.length; yyy++) {
                    ...
                    if (weapon.continuousReload) {
                        ...
                        if (weapon.canFire) {
                            if (weapon.lastFire == null) weapon.lastFire = 0;
                            let fireRateMulti = (weapon.name == "Evora" || weapon.name == "Veyron" ? (weapon.firedTime/1000 == 1.5 ? .4 : 1) : (weapon.firedTime / 16000));
                ...
                if (robot.ability && (robot.ability.name == "Phase Shift" ? true : !robot.isFREEZE)) {
                    ...
                    } else if (robot.target && (robot.ability.name == "Divine Judgement" || robot.ability.name == "Cold Pulse" || robot.ability.name == "Retribution")) {
                        let lowestRange = robot.weapons.sort((a, b) => a.range - b.range)[0];
                        if (Math.hypot(robot.y - robot.target.y, robot.x - robot.target.x) <= lowestRange.range && Math.hypot(robot.y - robot.target.y, robot.x - robot.target.x) <= (robot.ability.name == "Retribution" ? 800 : robot.ability.name == "Divine Judgement" ? 1800 : 1200) + robot.target.scale) {
                            robot.doUseAbility = true;
                        } else {
                            robot.doUseAbility = false;
                        }
