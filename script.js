(function () {
   ...
   - Bug Fixes<br><br>
            ...
            if (data.ability) {
                if (true) {
                    ...
                    if (data.ability.name == "Stealth Dash" || data.ability.name == "Phase Shift" || data.ability.name == "Dash" || data.ability.name == "Fortify") {
                        ...
                        if (data.ability.name == "Cold Pulse" || data.ability.name == "Dragon Flight" || data.ability.name == "Retribution" || data.ability.name == "Stampede") {
                            this.ability.dmg = data.ability.damageData ? data.ability.damageData.base : data.ability.dmg;
                            if (data.ability.name == "Stampede" || data.ability.name == "Dragon Flight" || data.ability.name == "Retribution") {
                                this.ability.lastingTime = data.ability.lastingTime;
    ...
    var shapeData = [{
        ...
        buyDesc: `
        <br><br>
        Cost: 750.0M Silver + 7.5K Gold + 4.0K Workshop Points
        `,
        healthData: {
            base: 160e3,
        ...
        ability: {
            ...
            damageData: {
                base: 240,
                level: [240, 270, 300, 330, 360, 390, 420, 450, 480, 510, 540, 570],
            },
            lastingTime: 12e3,
            reload: 8e3
        },
    ...
    document.getElementById("settingsToggle").onclick = function () {
        ...
        - SPACE while in lootbox animation, would speed it up
        </div>
        `;
    ...
    function drawWeaponDisplay(weapon, index, START_WEAPON, weaponslength) {
        ...
        document.getElementById("CURRENTINDEXX").innerHTML = `${index+1}/${weaponslength}`;
        document.getElementById("WEAPONdisplay").innerHTML = `
    ...
    function enterBarForShape(type, shape) {
        ...
        if (type == "health") {
            ...
            `;
        } else if (type == "ashield hp" && shape.ability.shieldHp) {
            ...
            let indexAdjusted = (((shape.ability.shieldHp + (shape.level == 12 ? maxspeed - MK1 : Shape.ability.shieldHpData.level[shape.level])) / maxspeed) * maxwidth) / maxwidth;
            text = `
            <div style="position: relative; width: ${maxwidth}px;">
            Shield Durability: ${abbreviateNumber(shape.ability.shieldHp)}
            <div style="display: ${Shape.ability.shieldHpData.level[shape.level] ? "block" : shape.level == 12 ? "block" : "none"};position: absolute; top: 0px; right: 0px; color: #00ff00;">
            +${shape.level == 12 ? abbreviateNumber(maxspeed - MK1) : abbreviateNumber(Shape.ability.shieldHpData.level[shape.level])}
            </div>
            </div>
            <div style="position: relative; width: ${maxwidth}px; height: 3px; background-color: #fff;">
            <div style="position: absolute; top: 0px; left: 0px; height: 3px; width: ${indexAdjusted * 100}%; background-color: #02de02;">
    ...
    function addGrayDamage(shape, amount) {
        ...
        shape.grayDamage += Math.abs(amount * (shape.effects.find(e => e.name == "stampede") ? .01 : shape.name == "Orange Circle" ? .25 : .4) * grayDamageMulti);
    }
    function updatePlayer(delta) {
        ...
        if (Player.robotIndex >= 0) {
            ...
            for (let i = 0; i < robot.shields.length; i++) {
                let shield = robot.shields[i];
                if (shield && shield.health <= 0 && !shield.regen) {
            ...
            for (let tt = 0; tt < robot.effects.length; tt++) {
                ...
                if (effect) {
                    ...
                    if (robot.effects.find(e => e.name == "phase shift") ? (effect.name == "phase shift" || effect.name == "last stand" || effect.name == "stealth dash") : true) {
            ...
            if (!robot.isFREEZE && (keysPressed[87] || keysPressed[65] || keysPressed[83] || keysPressed[68])) {
                ...
                if (robot.effects.find(e => e.name == "stampede" && e.lastTime > 2e3)) {
            ...
            if(robot.isFREEZE && robot.effects.find(e => e.name == "stampede")) {
                let effect = robot.effects.find(e => e.name == "stampede");
            ...
            if (robot.ability && (robot.ability.name == "Phase Shift" ? true : !robot.isFREEZE)) {
                ...
                if (keysPressed[69]) {
                    ...
                    if (robot.ability.name == "Stampede" || robot.ability.name == "Stealth" || robot.ability.name == "Retribution" || robot.ability.name == "Ultimate Defense" || robot.ability.name == "Self Heal" || robot.ability.name == "Dragon Flight" || robot.ability.name == "Shield Regeneration" || robot.ability.name == "Full Action") {
            ...
            for (let t = 0; t < touchBullets.length; t++) {
                if (touchBullets[t].dmg > 0) {
                    ...
                    if (touchBullets[t].freezeEffect) {
        ...
        if (player.gameMode >= 0) {
            ...
            for(let i = 0; i < KILLLOGS.length; i++) {
                ...
                if(log.weapon && log.weapon.level) {
                    document.getElementById("LOGGER").innerHTML += `
                    <div>
                    <span style="color: ${log.attacker == "player" ? "#0f0" : "#fff"};">${log.attacker}</span> <span style="color: ${log.weapon.level == 13 ? `#fff` : `#000`};">[${log.weapon.name}${log.weapon.level == 13 ? ` <span style="color: #0f0">MK2</span>` : ""}]</span> <span style="color: #fff;">${log.victim}</span>
                    </div>
                    `;
                }
            }
