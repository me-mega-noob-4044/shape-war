(function () {
        ...
        constructor(data, slot, dontSID) {
            ...
            if (data.ability) {
                if (true) {
                    ...
                    if (data.ability.name == "Cold Pulse" || data.ability.name == "Dragon Flight") {
                        this.ability.dmg = data.ability.damageData ? data.ability.damageData.base : data.ability.dmg;
                        if(data.ability.name == "Dragon Flight") {
    ...
    function resize() {
        ...
        document.getElementById("PROAD3").style.top = (window.innerHeight/2-200) + "px";
        document.getElementById("PROAD3").style.left = (window.innerWidth/2-200) + "px";
        document.getElementById("PROAD2").style.top = (window.innerHeight/2-200) + "px";
        document.getElementById("PROAD2").style.left = (window.innerWidth/2-200) + "px";
    ...
    function enterBarForShape(type, shape) {
        ...
        if (type == "health") {
            ...
            let indexAdjusted = (((shape.ability.dotDamage + Shape.ability.dotData.level[shape.level]) / maxspeed) * maxwidth) / maxwidth;
            text = `
            <div style="position: relative; width: ${maxwidth}px;">
            Ability DOT Damage: ${abbreviateNumber(shape.ability.dotDamage)}
            <div style="display: ${Shape.ability.dotData.level[shape.level] ? "block" : "none"};position: absolute; top: 0px; right: 0px; color: #00ff00;">
            +${abbreviateNumber(Shape.ability.dotData.level[shape.level])}
            </div>
            </div>
            <div style="position: relative; width: ${maxwidth}px; height: 3px; background-color: #fff;">
            <div style="position: absolute; top: 0px; left: 0px; height: 3px; width: ${indexAdjusted * 100}%; background-color: #02de02;">
    ...
    function updatePlayer(delta) {
        ...
        if (Player.robotIndex >= 0) {
            ...
            document.getElementById("weaponThing").innerHTML = "";//robot.dotDamage
            ...
            <div style="position: absolute; top: 0px; right: ${100-(100 * (robot.health / robot.maxhealth))}%; width: ${100 * (Math.min(robot.dotDamage, robot.health) / robot.maxhealth)}%; height: 100%; background-color: #ffff00;">
            </div>
            ...
            for (let i = 0; i < robot.shields.length; i++) {
                ...
                if (shield.regen) {
                    ...
                    if (Date.now() - shield.lastRegen >= 5e3 * robot.shieldRegen) {
                ...
                if (shield.health > 0) {
                    let touchBullets = projectiles.filter(e => !e.isAlly && Math.hypot(e.y - robot.y, e.x - robot.x) <=  20 + (e.scale*1.5) + robot.scale);
            ...
            if (robot.ability && !robot.isFREEZE) {
                ...
                if (keysPressed[69]) {
                    ...
                    if (robot.ability.name == "Self Heal" || robot.ability.name == "Dragon Flight" ||  robot.ability.name == "Shield Regeneration" || robot.ability.name == "Full Action") {
                ...
                if (robot.abilityReload == 0 && robot.abilityLast == 0) {
                    robot.scale = Math.max(robot.scale - (0.04*delta), robot.oldScale);
                ...
                if (robot.abilityLast && robot.abilityLast > 0) {
                    ...
                    if (robot.ability.name == "Self Heal") {
                        ...
                        robot.scale = Math.min(robot.scale + (0.04*delta), robot.oldScale*2);
                        deltaTimer -= delta;
                        if(deltaTimer <= 0) {
                            deltaTimer = 50;
        ...
        for (let i = 0; i < projectiles.length; i++) {
            ...
            if (bullet && isOnScreen(bullet.x - offset.x, bullet.y - offset.y, bullet.scale)) {
                ...
                if (image) {
                    ...
                    renderCircle(0, 0, bullet.scale, ctx, false, false, bullet.color.includes("#") ? bullet.color : "#808080", "#000", 1);
                }
                ctx.restore();
            }
        }
        ...
        for (let i = 1; i < players.length; i++) {
            ...
            if (robot) {
                ...
                for (let tt = 0; tt < robot.shields.length; tt++) {
                    ...
                    if (shield.health > 0) {
                        let touchBullets = projectiles.filter(e => e.isAlly != players[i].isAlly && Math.hypot(e.y - robot.y, e.x - robot.x) <= 20 + (e.scale*1.5) + robot.scale);
                        for (let t = 0; t < touchBullets.length; t++) {
                            shieldEffects.push({
                                x: robot.x,
                                y: robot.y,
                ...
                if (robot.ability && !robot.isFREEZE) {
                    ...
                    }else if(robot.ability.name == "Shield Regeneration" || robot.ability.name == "Full Action") {
                        if(SHIELDREGENPOWER == true && robot.ability.name == "Shield Regeneration") {
                            robot.doUseAbility = true;
                        }else if(robot.health < robot.maxhealth && robot.ability.name == "Full Action") {
                            robot.doUseAbility = true;
                        }else {
                            robot.doUseAbility = false;
                        }
                    }else if(robot.target && robot.ability.name == "Cold Pulse") {
                        let lowestRange = robot.weapons.sort((a, b) => a.range - b.range)[0];
