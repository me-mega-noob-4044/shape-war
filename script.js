(function () {
    ...
    class shape {
        constructor(data, slot, dontSID) {
            ...
            this.ability = null;
            if (data.ability) {
                if (data.ability.name == "Self Heal" || data.ability.name == "Shield Regeneration" || data.ability.name == "Full Action" || data.ability.name == "Cold Pulse") {
                    this.ability = {
        ...
        if (Player.robotIndex >= 0) {
            ...
            if (robot.ability && !robot.isFREEZE) {
                ...
                if (keysPressed[69]) {
                    ...
                    if (robot.ability.name == "Self Heal" || robot.ability.name == "Shield Regeneration" || robot.ability.name == "Full Action") {
    ...
    function updateBullets(delta) {
        for (let i = 0; i < projectiles.length; i++) {
            ...
            if (bullet) {
                let asd = buildings.find(e => e.name.includes("rect") && circleSquareCollision(bullet, e, true));
                ...
                if (robot.ability && !robot.isFREEZE) {
                    if(robot.ability.name == "Self Heal") {
                        ...
                        if(SHIELDREGENPOWER == true && robot.ability.name == "Shield Regeneration") {
                            robot.doUseAbility = true;
                            console.log(robot.ability.name);
    ...
    function circleSquareCollision(circle, square, predict) {
        ...
        if(distance <= circle.scale) {
            return true;
        }else if(predict) {
            let d = Math.hypot(circle.lastY - circle.y, circle.lastX - circle.x);
            let Px = Math.max(square.x, Math.min(circle.lastX, square.x + square.width));
            let Py = Math.max(square.y, Math.min(circle.lastY, square.y + square.height));
            for(let i = 1; i <= 10; i++) {
                let div = (d/10)*i;
                if(Math.hypot(circle.lastY - Py, circle.lastX - Px) + div <= circle.scale) {
                    return true;
                }
            }
            return false;
