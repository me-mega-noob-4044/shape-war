(function () {
    ...
    class shape {
        constructor(data, slot, dontSID) {
            ...
            this.ability = null;
            if (data.ability) {
                if (data.ability.name == "Self Heal" || data.ability.name == "Full Action") {
                    this.ability = {
    ...
    var shapeData = [{
        ...
        ability: {
            name: "Full Action",
            desc: `
            <strong>Full Action</strong> gains 60 DP (37.5% resistance) for 4 seconds, after that it cooldown for 20 seconds.
            `,
        ...
        ability: {
            name: "Full Action",
            desc: `
            <strong>Full Action</strong> gains 200 DP (66.7% resistance) for 12 seconds, after that it cooldown for 24 seconds.
            `,
            iconSource: "./images/full_action.png",
            lastingTime: 12e3,
            reload: 24e3
    ...
    var weaponData = [{
        ...
        desc: `
        Light laser weapon.
        Special targeting allows the weapon to target 4 enemies at once.
        Ignores mouse cursor and targets nearest 4 enemies.
        `,
        buyDesc: `
        <br><br>
        Cost: 1.75K Gold
    ...
    function enterBarForWeapon(type, weapon) {
        ...
        let maxwidth = window.innerWidth >= 1360 ? 500 : 250;
        if (type == "damage") {
    ...
    function customizeWeapon(weapon, shape, slot) {
        if (true) {
            ...
            if (weapon == "Light" || weapon == "Heavy") {
                filteredWeapons = player.weapons.filter(e => e.owner == null && e.type == weapon).sort((a, b) => b.level - a.level);
            } else {
                filteredWeapons = player.weapons.filter(e => e.owner == null && e.type == weapon.type).sort((a, b) => b.level - a.level);
            }
            let currentIndex = 0;
    ...
    console.log(weaponData.findIndex(e => e.name == "Landslide"));
    var lootboxes = [{
    ...
    function doLootboxAnimation(rewards, reward) {
        ...
        let indexx = 0;//shape weapon
        let rewardIndex = 0;
        for(let tt = 0; tt < 6; tt++) {
            for(let i = 0; i < rewards.length; i++) {
                document.getElementById("rewardAnimation").innerHTML += `
                <div id="${`${rewards[i].index}nothign${indexx}${rewards[i].type}`}" style="position: absolute; width: 200px; height: 200px; top: 0px; left: ${indexx*200}px; background-color: ${indexx % 2 == 0 ? "rgb(255, 255, 255, 0.4)" : "rgb(0, 0, 0, 0.2)"}; background-size: 200px 200px; ${(rewards[i].ag || rewards[i].au ? "background-image: url('" + (rewards[i].ag ? agSource : auSource) + "')" : "")}">
                <div style="font-size: 20px; display: ${rewards[i].ag ? "block" : rewards[i].au ? "block" : "none"}; position: absolute; bottom: 5px; right: 5px;">
        ...
        for(let i = 0; i < rewards.length; i++) {
            document.getElementById("rewardAnimation").innerHTML += `
            <div id="${rewards[i] == reward ? `${rewards[i].index}THEREWARD${indexx}${rewards[i].type}` : `${rewards[i].index}nothign${indexx}${rewards[i].type}`}" style="position: absolute; width: 200px; height: 200px; top: 0px; left: ${indexx*200}px; background-color: ${rewards[i] == reward ? "rgb(255, 215, 0, 0.9)" : indexx % 2 == 0 ? "rgb(255, 255, 255, 0.4)" : "rgb(0, 0, 0, 0.2)"}; background-size: 200px 200px; ${(rewards[i].ag || rewards[i].au ? "background-image: url('" + (rewards[i].ag ? agSource : auSource) + "')" : "")}">
            <div style="font-size: 20px; display: ${rewards[i].ag ? "block" : rewards[i].au ? "block" : "none"}; position: absolute; bottom: 5px; right: 5px;">
            ${abbreviateNumber(rewards[i].amount)}
            </div>
            </div>
        ...
        if (player.gameMode >= 0) {
            ...
            if(keysPressed[32] && players.length && players[0] && players[0].robots && players[0].robots.length) {
                ...
                if(robot) {
                    for(let i = 0; i < robot.weapons.length; i++) {
                        let weapon = robot.weapons[i];
                        if(weapon.name == "Hawk" && weapon.canFire) {
                            let nearestEnemies = enemies.filter(e => Math.hypot(e.y - robot.y, e.x - robot.x) <= weapon.range).sort((a, b) => Math.hypot(a.y - robot.y, a.x - robot.x) - Math.hypot(b.y - robot.y, b.x - robot.x));
                            ctx.save();
                            ctx.lineWidth = 16;
                            ...
                            for(let ttt = 0; ttt < 4; ttt++) {
                                ...
                                if(enemy) {
                                    if(ttt == 0) {
                                        ...
                                        ctx.moveTo(enemy.x - offset.x, enemy.y - offset.y);
                                        if(enemy2 && ttt + 1 < 4) {
