(function () {
    ...
    function enterBarForWeapon(type, weapon) {
        ...
        if (type == "damage") {
            ...
            text = `
            <div style="position: relative; width: ${maxwidth}px;">
            Damage: ${abbreviateNumber(weapon.dotDamage)}
            ...
            for (let i = 0; i < robot.weapons.length; i++) {
                ...
                if (weapon.continuousReload) {
                    ...
                    if (weapon.canFire) {
                        ...
                        if (keysPressed[32] && Date.now() - weapon.lastFire >= weapon.fireRate * (weapon.name == "Atomizer" ? 1 + fireRateMulti : 1)) {
                            console.log(weapon.fireRate * (weapon.name == "Atomizer" || weapon.name == "Nucleon" ? 1 + fireRateMulti : 1));
                            weapon.lastFire = Date.now();
                            if(weapon.name == "Hawk" || weapon.name == "Eagle") {
                                let nearestEnemies = enemies.filter(e => Math.hypot(e.y - robot.y, e.x - robot.x) <= weapon.range + e.scale).sort((a, b) => Math.hypot(a.y - robot.y, a.x - robot.x) - Math.hypot(b.y - robot.y, b.x - robot.x));
        ...
        for (let i = 0; i < enemies.length; i++) {
            ...
            if (enemy && playerRobot) {
                ...
                if(!enemy.isFREEZE) {
                    ...
                    } else if (enemy.name == "Level 1 Boss") {
                    ...
                    } else if (enemy.name == "Level 2 Boss") {
                        let angle = Math.atan2(playerRobot.y - enemy.y, playerRobot.x - enemy.x);
                        let percent = (enemy.health / enemy.maxhealth)*100;
                        enemy.velx += Math.cos(angle + (percent <= 10 ? Math.PI : 0)) * (moveSpeed * delta);
                        enemy.vely += Math.sin(angle + (percent <= 10 ? Math.PI : 0)) * (moveSpeed * delta);
