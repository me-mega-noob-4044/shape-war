   ...
   - Bug Fixes<br><br>
    ...
    class shape {
        constructor(data, slot, dontSID) {
            ...
            this.y = 0;
            this.velx = 0;
            this.betterDescription = data.betterDescription;
...
*/
    ...
    function customizeShape(shape) {
        ...
        <div id="showBetterDescription" style="display: ${shape.betterDescription ? "block" : "none"};position: absolute; color: #fff; cursor: pointer; top: 15px; text-align: center; right: 10px; border-radius: 100%; width: 20px; height: 20px; background-color: rgb(0, 0, 0, 0.2);">
        i
        </div>
        <div style="position: absolute; top: 45px; left: 0px;">
        <hr style="position: absolute; top: 0px; left: 0px; color: #fff; width: 325px;">
                            ...
                            if (isAlly) {
                                if (obj.ENEMYPOINTS) {
                                    ...
                                    if (obj.ALLYPOINTS > 5e3) {
                                        if (robot.isMe && obj.LASTALLYPOINTS < 5e3) {
    ...
    function doOtherBotStuff(robot, isAlly) {
        ...
        if (robot.ability) {
            ...
            if (["Cold Pulse", "Divine Judgement", "Full Action", "Stealth", "Domain Expansion: Infinite Void", "Paladin", "Dragon Flight", "Phase Shift", "Self Heal", "Retribution"].includes(abilityName) && Date.now() - robot.damagedTime <= 50) {
    ...
    function changeHealth(object, { amount, graydmg, isDotDamage, defensePointsBypass, bypassReflector, damageTypeName }, isPlayer, doer, noDefense) {
        try {
            ...
            if (doer && doer.isMe && (object.turnDmgIntoDOT ? isDotDamage : true)) {
    ...
    function antiTier4_5(bullet, enemy) {
        ...
        if (enemy.maxhealth >= 350e3) {
            ...
            amount += Math.min(Math.floor(mm / 25e3) * .2, 750);
        }
                ...
                for (let t = 0; t < touchBullets.length; t++) {
                    ...
                    if (touchBullets[t].dmg > 0) {
                        ...
                        if (touchBullets[t].dotEffect) {
                            if (touchBullets[t].dotEffect.resetDOT) {
                ...
                for (let i = 0; i < buildings.length; i++) {
                    ...
                    if (obj.name == "coll rect") {
                        ...
                        if (distance <= 25 + robot.scale) {
                            if (obj.middleX) {
                                if ((TARGET.x <= obj.x && robot.x >= obj.x) || (TARGET.x >= obj.x && robot.x <= obj.x)) {
