(function () {
   ...
   - Bug Fixes<br><br>
        ...
        constructor(data, slot, dontSID) {
            ...
            if (data.ability) {
                if (true) {
                    ...
                    if (data.ability.name == "Stealth Dash" || data.ability.name == "Phase Shift" || data.ability.name == "Dash" || data.ability.name == "Fortify") {
    ...
    var shapeData = [{
        ...
        healthData: {
            base: 278.4e3,
            level: [0, 35.2e3, 35.2e3, 35.2e3, 38.4e3, 38.4e3, 38.4e3, 41.6e3, 41.6e3, 41.6e3, 44.8e3, 48e3]
        },
        hardpoints: {
            light: 1,
            heavy: 2
        ...
        ability: {
            ...
            damageData: {
                base: 4e3,
                level: [0, 400, 400, 600, 600, 800, 800, 1e3, 1e3, 2e3, 2e3, 5e3],
            },
            lastingTime: 2e3,
            reload: 5e3
        },
    ...
    function enterBarForWeapon(type, weapon) {
        ...
        if (type == "damage") {
            ...
            let mk2 = maxdmg;
            maxdmg *= 1.5;
            console.log(maxdmg, mk2, mk1, mk1 + (mk2-mk1));
    ...
    function upgradeWeapon(weapon, currentIndex, old_WEAPON, noEz, isBOT) {
        ...
        if (weapon.level < 12) {
            ...
            weapon.dmg *= isBOT ? 1.2 : weapon.level == 13 ? 1.5 : 1.2;
        }
        if (Weapon.dotData) {
            if (weapon.level < 12) {
                ...
                weapon.dotDamage *= isBOT ? 1.2 : weapon.level == 13 ? 1.5 : 1.2;
            }
        }
        weapon.level++;
    ...
    document.getElementById("goToBattle").onclick = function () {
        ...
        document.getElementById("multiPlayerToggle").style.display = "none";
    ...
    document.getElementById("goBackToHanger").onclick = function () {
        ...
        document.getElementById("multiPlayerToggle").style.display = "block";
    ...
    function upgradeShape(shape, noEz, isBOT) {
        ...
        if (shape.level < 12) {
            ...
            shape.health *= isBOT ? 1.2 : shape.level == 13 ? 1.5 : 1.2;
            shape.maxhealth *= isBOT ? 1.2 : shape.level == 13 ? 1.5 : 1.2;
        ...
        if (shape.ability) {
            if (shape.ability.shieldHp) {
                ...
                if (shape.level < 12) {
                    ...
                    shape.ability.shieldHp *= isBOT ? 1.2 : shape.level == 13 ? 1.5 : 1.2;
                }
            }
            if (shape.ability.dmg) {
                ...
                if (shape.level < 12) {
                    ...
                    shape.ability.dmg *= isBOT ? 1.2 : shape.level == 13 ? 1.5 : 1.2;
                }
            }
            if (shape.ability.dotDamage) {
                ...
                if (shape.level < 12) {
                    ...
                    shape.ability.dotDamage *= isBOT ? 1.2 : shape.level == 13 ? 1.5 : 1.2;
                }
            }
        }
        ...
        if ((shape.isMe ? true : shape.target)) {
            ...
            if (weapon.name == "Punisher") {
                ...
                projectiles.push({
                    ...
                    color: "./images/purple_bullet.png",
                    owner: shape,
                    weaponOwner: {
                        name: weapon.name,
                        level: weapon.level
    ...
    function addGrayDamage(shape, amount) {
        ...
        shape.grayDamage += Math.abs(amount * (shape.effects && shape.effects.find(e => e.name == "stampede") ? .05 : shape.name == "Orange Circle" ? .25 : .4) * grayDamageMulti);
    }
    function updatePlayer(delta) {
        ...
        if (Player.robotIndex >= 0) {
            ...
            for (let tt = 0; tt < robot.effects.length; tt++) {
                ...
                if (effect) {
                    ...
                    if (hasPhaseShift ? (effect.name == "phase shift" || effect.name == "last stand" || effect.name == "stealth dash") : true) {
                        ...
                        if ((effect.name == "dot" || effect.name == "freezing") && effectNames[effect.name].orginal < effect.lastTime) {
                    ...
                    if(effect.lastTime > 0) {
                        if (effect.name == "cold pulse") {
                            ...
                            if(effect.lastTime > 2e3) slowdownMulti += 1.5;
            ...
            if (!robot.isFREEZE && (keysPressed[87] || keysPressed[65] || keysPressed[83] || keysPressed[68])) {
                ...
                if (hasStampede && hasStampede.lastTime > 2e3) {
                                ...
                                for (let t = 1; t < players.length; t++) {
                                    ...
                                    if (player && players[t].isAlly != players[0].isAlly) {
                                        if (Math.hypot(player.y - robot.y, player.x - robot.x) <= weapon.range + player.scale) {
                                            ...
                                            changeHealth(player, -weapon.dmg, true, robot);
            ...
            if (robot.ability && (robot.ability.name == "Phase Shift" ? true : !robot.isFREEZE)) {
                ...
                if (keysPressed[69]) {
                    ...
                    if (robot.ability.name == "Overload" || robot.ability.name == "Stampede" || robot.ability.name == "Stealth" || robot.ability.name == "Retribution" || robot.ability.name == "Ultimate Defense" || robot.ability.name == "Self Heal" || robot.ability.name == "Dragon Flight" || robot.ability.name == "Shield Regeneration" || robot.ability.name == "Full Action") {
            ...
            for (let t = 0; t < touchBullets.length; t++) {
                ...
                if(robot.health <= 0 && hadHealth) {
                    addLogger(touchBullets[t].owner ? touchBullets[t].owner.PLAYERNAME || "unknown bot" : "unknown bot", touchBullets[t].weaponOwner, "player");
                }
                touchBullets[t].dmg = 0;
                touchBullets[t].kill = true;
            }
        ...
        try {
            ...
            if (amount != 0) {
                ...
                if (object.turnDmgIntoDOT && amount < 0 && doer && !noDefense) {
                    let duration = object.effects.find(e => e.name == "stampede") ? 7 : object.name == "Orange Circle" ? 30 : 10;
                    if(object.effects.filter(e => e.name == "defense points").length < 500 && object.effects.find(e => e.name == "stampede")) {
                        object.effects.push({
                            name: "defense points",
                            amount: 1,
                            lastTime: 20e3
            ...
            if (robot) {
                        ...
                        if(effect.lastTime > 0) {
                            if (effect.name == "cold pulse") {
                                ...
                                if (effect.dmgOverTime <= 0) {
                                    ...
                                    let hadHealth = robot.health > 0;
                                    changeHealth(robot, -effect.dmg, "LMAOLMAOLMAOLMAO", null, true);
                                    if(robot.health <= 0 && hadHealth) {
                                        addLogger(effect.owner ? effect.owner.PLAYERNAME || "unknown bot" : "unknown bot", {
                                            name: "DOT EFFECT",
                                            level: 1
                                ...
                                for (let t = 0; t < players.length; t++) {
                                    ...
                                    if (player && players[t].isAlly != players[i].isAlly) {
                                        if (Math.hypot(player.y - robot.y, player.x - robot.x) <= abilityRange + player.scale) {
                                            ...
                                            let hasHealth = player.health > 0;
                                            changeHealth(player, -robot.ability.dmg, true);
                ...
                for (let t = 0; t < touchBullets.length; t++) {
                    if (touchBullets[t].dmg > 0) {
                        ...
                        if (touchBullets[t].freezeEffect && !robot.effects.find(e => e.name == "cold pulse")) {
                            for(let asdasd = 0; asdasd < touchBullets[t].freezeEffect.power; asdasd++) {
                                robot.effects.push({
                                    name: touchBullets[t].freezeEffect.name,
                                    lastTime: touchBullets[t].freezeEffect.last
                    ...
                    if(robot.health <= 0 && hadHealth) {
                        addLogger(touchBullets[t].owner ? touchBullets[t].owner.isMe ? "player" : touchBullets[t].owner.PLAYERNAME || "unknown bot" : "unknown bot", touchBullets[t].weaponOwner, robot.PLAYERNAME);
                    }
                    touchBullets[t].dmg = 0;
                    touchBullets[t].kill = true;
                }
                ...
                if (robot.ability && (robot.ability.name == "Phase Shift" ? true : !robot.isFREEZE)) {
                    ...
                    } else if (robot.ability.name == "Dragon Flight" || robot.ability.name == "Overload") {
                        robot.doUseAbility = true;
                    } else if (robot.ability.maxcharge) {
                        robot.doUseAbility = true;
                    } else {
                    ...
                    if (robot.doUseAbility) {
                        if (robot.abilityLast == null) robot.abilityLast = 0;
                        if (robot.abilityReload == null) robot.abilityReload = 0;
                        if (robot.ability.name == "Overload" || robot.ability.name == "Stealth" || robot.ability.name == "Retribution" || robot.ability.name == "Ultimate Defense" || robot.ability.name == "Self Heal" || robot.ability.name == "Dragon Flight" || robot.ability.name == "Shield Regeneration" || robot.ability.name == "Full Action") {
                            if (robot.abilityReload == 0 && robot.abilityLast == 0) {
                                robot.abilityLast = robot.ability.lastingTime;
        ...
        if (player.gameMode >= 0) {
            ...
            for (let i = 0; i < bombeffect.length; i++) {
                ...
                if (effect) {
                    ...
                    let color = effect.color ? effect.color == "stamp" ? "rgb(0, 0, 255, 0)" : "rgb(255, 255, 255, 0.3)" : "rgb(255, 0, 0, 0.3)";
                    let color2 = effect.color ? effect.color == "stamp" ? "rgb(0, 0, 255, 0.6)" : "rgb(255, 255, 255, 0.6)" : "rgb(255, 0, 0, 0.6)";
                    ctx.save();
            ...
            for(let i = 0; i < KILLLOGS.length; i++) {
                ...
                if(log.weapon && log.weapon.level) {
                    document.getElementById("LOGGER").innerHTML += `
                    <div>
                    <span style="color: ${log.attacker == "player" ? "#0f0" : "#fff"};">${log.attacker}</span> <span style="color: ${log.weapon.level >= 13 ? `#fff` : `#000`};">[${log.weapon.name}${log.weapon.level >= 13 ? ` <span style="color: #${log.weapon.level == 13 ? "0f0" : log.weapon.level == 14 ? "ffff00" : "f00"}">${log.weapon.level == 13 ? "MK2" : log.weapon.level == 14 ? "MK3" : "MK4"}</span>` : ""}]</span> <span style="color: #fff;">${log.victim}</span>
                    </div>
                    `;
                }
            }
