(function () {
   ...
   - Bug Fixes<br><br>
    ...
    var moduleData = [{
        ...
        imageSource: "./images/last_stand.png",
        cost: {
            sliver: 25e6,
            gold: 5e3
        }
    ...
    function doLootboxAnimation(rewards, reward) {
        ...
        function animate() {
            ...
            tmpScroll += (10 - ((percent / 10) % 10)) * delta * (keysPressed[32] ? 4 : 1);
    ...
    function updatePlayer(delta) {
        ...
        if (Player.robotIndex >= 0) {
            ...
            for (let tt = 0; tt < robot.effects.length; tt++) {
                ...
                if (effect) {
                    ...
                    if (effect.name == "cold pulse") {
                        ...
                        if (effectNames["last stand"] == null) {
                            effectNames["last stand"] = {
                                amount: 1,
                                duration: effect.lastTime,
                                orginal: effect.oldLastTime
                            };
                        } else {
                            effectNames["last stand"].amount++;
                            effectNames["last stand"].duration = effect.lastTime;
                        }
            ...
            if (!robot.usedLastStand && robot.health <= (robot.lastStandThreshold * robot.maxhealth)) {
                ...
                robot.effects.push({
                    name: "last stand",
                    placement: (robot.lastStandThreshold * robot.maxhealth),
            ...
            for (let i in effectNames) {
                ...
                <div style="position: absolute; top: 0px; left: 5px; height: 40px; width: 40px; background-size: 40px 40px; background-image: url('${i == "dot" ? "./images/viper.png" : i == "last stand" ? "./images/last_stand.png" : "./images/cold_pulse.png"}');">
                </div>
    ...
    function changeHealth(object, amount, isPlayer, doer, noDefense) {
        try {
            ...
            if (!object || object.health <= 0) return;
    ...
    function checkIfHit(proj, enemy) {
        if(Math.hypot(proj.y - enemy.y, proj.x - enemy.x) <= proj.scale + enemy.scale) {
            return true;
        }
        ...
        for (let i = 1; i <= 10; i++) {
            let div = (d / 10) * i;
            if (Math.hypot(proj.y - enemy.y, proj.x - enemy.x) + div <= proj.scale + enemy.scale) {
                return true;
            }
        }
        return false;
